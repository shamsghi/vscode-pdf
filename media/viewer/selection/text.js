...
export function mergeSelectionHighlightRects(items) {
    if (items.length === 0) {
        return [];
    }
    const merged = [];
...
export function getSpanBounds(span) {
...
function collectWordCandidates(textLayer, layerRect) {
    const spanIds = new WeakMap();
    let nextSpanId = 0;
    const candidates = [];
...
function expandRectangleSelection(candidates, selected, selectionBounds) {
    const paddedBounds = {
...
function groupCandidatesByLine(candidates) {
    const lines = [];
...
function spansSelectionBand(item, bounds) {
    return boxesOverlap(itemToBounds(item), bounds);
}
function itemToBounds(item) {
    return {
        left: item.left,
        top: item.top,
        right: item.right,
...
function textItemFromRect(text, rect, layerRect) {
    return {
        text,
...
function intersectsSelection(points, item) {
...
function drawMergedSelectionHighlights(textLayer, items) {
